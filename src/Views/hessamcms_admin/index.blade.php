    ...
    @forelse($post_translations as $post)
        <div class="card m-4" style="">
            <div class="card-body">
                    ...
                    <dd class="">
                        @if(count($post->post->categories))
                            @foreach($post->post->categories as $category)
                                <a class='btn btn-outline-secondary btn-sm m-1' href='{{!empty($category->categoryTranslations->where('lang_id' , $language_id)->first()) ?$category->categoryTranslations->where('lang_id' , $language_id)->first()->edit_url():''}}'>
                                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i>

                                    {{!empty($category->categoryTranslations->where('lang_id' , $language_id)->first()) ? $category->categoryTranslations->where('lang_id' , $language_id)->first()->category_name : ''}}
                                </a>
                            @endforeach
                        @else No Categories
                        @endif
                ...
                <a href="{{$post->url($locale)}}" class="card-link btn btn-outline-secondary"><i class="fa fa-file-text-o"
                                                                                                                       aria-hidden="true"></i>
                    View Post</a>
                <a href="{{$locale=='en' || $language_id == 1 ? $post->edit_url() : $post->edit_toggle_url() . '?selected_lang=' . $language_id}}" class="card-link btn btn-primary">
                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                    Edit Post</a>
                <form onsubmit="return confirm('Are you sure you want to delete this blog post?\n You cannot undo this action!');"
                      method='post' action='{{route("hessamcms.admin.destroy_post", $post->post_id)}}' class='float-right'>
                    @csrf
